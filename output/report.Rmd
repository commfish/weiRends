...
Based upon all years of data, the 95% mean passage weir removal date is `r tbls_egegik$run_through$date`. 
This is the date that the weir must be operated through, i.e., removal could occur on the following day.

```{r pred, fig.cap="Predicted cumulative escapements by year for the Egegik River. Filled circles indicate 95\\% of the run has passed the weir. The vertical line is the mean date when 95\\% of the run has passed the weir."}
...
```{r risk, fig.cap="The percent of the run that is missed at a given risk level, e.g., 4\\% of the run will be missed 5\\% of the time using a five day 1\\% rule for the Egegik River."}
...
```{r pred-koot, fig.cap="Predicted cumulative escapements by year for the Chilkoot River. Filled circles indicate 95\\% of the run has passed the weir. The vertical line is the mean date when 95\\% of the run has passed the weir."}
...
Note that there is a 50% chance of capturing 95-96% of the run, which is slightly lower than the expected 95% based upon the mean passage date stated previously.
...
```{r risk-koot, fig.cap="The percent of the run that will be missed at a given risk level, e.g., 15\\% of the run will be missed 5\\% of the time using a five day 1\\% rule for the Chilkoot River."}
...
```{r pred-situk, fig.cap="Predicted cumulative escapements by year for the Situk River. Filled circles indicate 95\\% of the run has passed the weir. The vertical line is the mean date when 95\\% of the run has passed the weir."}
knitr::include_graphics(here("figs/situk/pred_plot.png"))
```

```{r pred-decade-situk, fig.cap="Predicted cumulative escapements by year and decade for the Situk River. Filled circles indicate 95\\% of the run has passed the weir. The vertical line is the mean date when 95\\% of the run has passed the weir."}
