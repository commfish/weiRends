...
Based upon median removal dates presented in Table \@ref(tab:remove-koot), there is a 95% chance of capturing 89% of the total run, depending on which weir removal rule (# of days) is implemented (Table \@ref(tab:chance-koot)).
Note that there is a 50% chance of capturing 99% of the run, which is slightly lower than the expected 95% based upon the mean passage date stated previously.
...
Note that there is a 50% chance of capturing 95% of the run, e.g., the mean 95% passage date stated previously.
...
```{r risk-situk, fig.cap="The percent of risk that a given \\% of the run is missed, e.g., 10\\% of the run will be missed 5\\% of the time using a five day 1\\% rule for the Situk River."}
...
```{r risk05-situk, fig.cap="he percent of the run that will be missed at a given risk level, e.g., 5\\% chance that 4\\% of the run will be missed using a five day 0.05\\% rule for the Situk River."}
