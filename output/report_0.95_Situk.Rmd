...
```{r data}
fs::dir_ls(here::here('output/situk/1988_2021'), regexp = "\\.csv$") %>% 
  set_names(nm = (basename(.) %>% 
                    tools::file_path_sans_ext())) %>%
  map(read_csv) -> tbls_situk
fs::dir_ls(here::here('output/situk/2012_2021'), regexp = "\\.csv$") %>% 
  set_names(nm = (basename(.) %>% 
                    tools::file_path_sans_ext())) %>%
...
```{r param-koot, fig.cap="Parameter estimates from the Gompertz model for the Situk River."}
knitr::include_graphics(here("figs/situk/1988_2021/param_plot.png"))
```
...
knitr::include_graphics(here("figs/situk/1988_2021/pred_plot.png"))
```


## 1% Rule (*Hard Date* and *End Date*)
Using all the data in the time series (1988-2021), the 95th percentile date when 95% of the escapement has passed the weir  is Julian day `r tbls_situk$run_through$end_date` or `r tbls_situk$run_through$date` (the hard date). The earliest date the project can end is is Julian day 230 or 18 August (Julian day `r tbls_situk$run_through$end_date` plus one day; end date). Based upon these dates, there is a 95% chance of capturing roughly 94% of the total escapement for all weir removal rules (number of days; Table \@ref(tab:chance-koot)). In addition, there is a 50% chance of capturing about 98% of the escapement. This is also reflected in the percent risk as well (Figure \@ref(fig:risk-koot)); Figure \@ref(fig:risk-koot) reflects the inverse of Table \@ref(tab:chance-koot). For example, a 99% chance is the same as a 1% risk. About 7%  of the escapement is missed (i.e., 93% caught) at a given risk level (99% chance or 1% risk level) based on implementing a 5-day 1% rule. 

The projected median date that the project would end is Julian day 230 (18 August) for all weir removal rules (Table \@ref(tab:remove-koot)). The project end date was based on the entire time series and estimates of when the weir would have been removed had the 1% rule been used to manage weir operations. The maximum date of weir removal was Julian day 234 (22 August) using the 5-day 1% rule, Julian day 234 (22 August) using the 4-day 1% rule, and Julian day 233 (21 August) using the 3-day 1% rule (Table \@ref(tab:remove-koot)). 

Results varied little if only the last ten years of the time series are used in the analysis (see Appendix Table \@ref(tab:remove-koot-ten); Figure \@ref(fig:ten-year-plot)). Using the last ten years of the time series (2012-2021), the 95th percentile date when 95% of the escapement has passed the weir  is Julian day `r tbls_situk_ten_years$run_through$end_date` or `r tbls_situk_ten_years$run_through$date` (the hard date). The earliest date the project can end is is Julian day 230 or 18 August (Julian day `r tbls_situk_ten_years$run_through$end_date` plus one day; end date).  
...
```{r risk-koot, fig.cap="The percent of the escapement that will be missed at a given risk level, e.g., about 6.5\\% of the escapement will be missed 5\\% of the time using a 5-day 1\\% rule for the Situk River."}
knitr::include_graphics(here("figs/situk/1988_2021/remove_dates_risk_plot.png"))
```

# Conclusions
  1. The hard date, the date to which the weir must be operated through, is Julian day `r tbls_situk$run_through$end_date` or `r tbls_situk$run_through$date`. 
  
  3. Based upon the median weir removal date (end date; Julian day 230 or 18 August), using the number of days to implement the 1% rule for the Situk River, there is a 95% chance of capturing about 94% of the total escapement for the 5-day rule. 
...
knitr::include_graphics(here("figs/situk/1988_2021/pred_plot_1999year.png"))
```
...
knitr::include_graphics(here("figs/situk/1988_2021/pred_plot_2000year.png"))
```
...
knitr::include_graphics(here("figs/situk/1988_2021/pred_plot_2010year.png"))
```
...
knitr::include_graphics(here("figs/situk/1988_2021/fitted_plot.png"))
```
...
knitr::include_graphics(here("figs/situk/2012_2021/pred_plot.png"))
```

```{r remove-koot-ten, results = 'asis'}
